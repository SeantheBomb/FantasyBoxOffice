...
// .wrangler/tmp/pages-VcxuZb/functionsWorker-0.37835586222685447.mjs
...
__name(unb64, "unb64");
__name2(unb64, "unb64");
async function hashPasswordPBKDF2(password, saltB64 = null) {
...
async function onRequestPost({ request, env }) {
  ...
  const expires = new Date(Date.now() + 7 * 24 * 3600 * 1e3).toISOString();
  await env.DB.prepare(
    `INSERT INTO sessions (id, user_id, expires_at, created_at) VALUES (?, ?, ?, ?)`
  ).bind(sessionId, row.id, expires, now).run();
  return json(
    { ok: true },
  ...
  try {
    await env.DB.prepare(
      `INSERT INTO users (id, email, username, real_name, password_hash, password_salt, created_at)
       VALUES (?, ?, ?, ?, ?, ?, ?)`
    ).bind(userId, email.toLowerCase(), username, realName, hashB64, saltB64, now).run();
  } catch {
...
async function onRequest2(context) {
  ...
  if (!env.TMDB_TOKEN) {
    return json2({ error: "Server misconfigured: TMDB_TOKEN missing" }, 500);
  }
...
async function tmdbFetch(path, token, params = {}) {
  ...
  for (const [k, v] of Object.entries(params)) {
    if (v !== void 0 && v !== null && v !== "") u.searchParams.set(k, String(v));
  }
...
__name(tmdbFetch, "tmdbFetch");
__name2(tmdbFetch, "tmdbFetch");
function getUsTheatricalDateFromReleaseDates(releaseDatesJson) {
  ...
  const theatrical = rels.filter((x) => x.type === 3 && x.release_date).sort((a, b) => new Date(a.release_date) - new Date(b.release_date))[0];
  return theatrical?.release_date?.slice(0, 10) || null;
}
__name(getUsTheatricalDateFromReleaseDates, "getUsTheatricalDateFromReleaseDates");
__name2(getUsTheatricalDateFromReleaseDates, "getUsTheatricalDateFromReleaseDates");
...
async function fetchReleases({ token, from, to, minPopularity, strictDomestic }) {
  ...
  const candidates = all.filter((m) => (m.popularity ?? 0) >= minPopularity).filter((m) => inRange(m.primary_release_date || m.release_date, from, to));
  const out = [];
...
__name2(fetchReleases, "fetchReleases");
var routes = [
  {
    routePath: "/api/login",
    mountPath: "/api",
...
// .wrangler/tmp/bundle-AwpQ7c/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-AwpQ7c/middleware-loader.entry.ts
...
//# sourceMappingURL=functionsWorker-0.37835586222685447.js.map
