                  ...
                  <tbody>
                    {u.movies.map((m) => (
                      <tr key={m.tmdb_id} style={{ borderTop: "1px solid var(--fbo-border)", opacity: m.is_void ? 0.5 : 1 }}>
                        ...
                        <td style={{ whiteSpace: "nowrap", paddingRight: 8 }}>{m.status}</td>
                        <td style={{ textAlign: "right", whiteSpace: "nowrap", paddingRight: 8 }}>{fullCurrency(m.revenue)}</td>
                        <td style={{ textAlign: "right", whiteSpace: "nowrap", paddingRight: 8 }}>{fullCurrency(m.budget)}</td>
                        <td style={{ textAlign: "right", whiteSpace: "nowrap", color: profitColor(m.profit) }}>{fullCurrency(m.profit)}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
