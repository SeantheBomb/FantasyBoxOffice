  ...
  return (
    <div>
      ...
      <section style={{ marginBottom: 24 }}>
        ...
        <ProfitChart dates={history.dates} series={history.series} />
      </section>
      ...
      <section>
        ...
        {standings.users.map((u, idx) => (
          <div key={u.id} style={{ background: "white", border: "1px solid #eee", borderRadius: 8, padding: 16, marginBottom: 12 }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline" }}>
              <div><b>{idx + 1}. {u.username}</b> <span style={{ color: "#888" }}>({u.real_name})</span></div>
              <div style={{ fontSize: 20, fontWeight: 600, color: profitColor(u.total_profit) }}>
                {fullCurrency(u.total_profit)}
              </div>
            </div>
            {u.movies.length === 0 ? (
              <div style={{ color: "#999", marginTop: 8 }}>No movies yet.</div>
            ) : (
